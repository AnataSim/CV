...
const { getFirestore, collection, getDocs, doc, getDoc, setDoc, updateDoc, deleteDoc, query, where } = require('firebase/firestore');
...
const selfbot = require('./utils/selfbot');
...
// GET Leaderboards (Cakey Bot & CV$ Wealth)
...
// POST Rank Roles refresh/update
...
setTimeout(() => {
  if (state.db) {
    syncVolunteerablesFromFirestore().catch(() => {});
  }
}, 5000);
...
// ================== CHAT ENTIRES & CHANNELS ==================
try {
  const savedChans = db.loadCustomChannels();
  savedChans.forEach(chan => {
    if (!state.chatChannels.some(c => c.id === chan.id)) {
      state.chatChannels.push(chan);
    }
  });
} catch (e) {}

try {
  const savedMsgs = db.loadChatMessages();
  if (savedMsgs) {
    state.chatMessages = { ...state.chatMessages, ...savedMsgs };
  }
} catch (e) {}
...
app.post('/api/submissions/approve', requireClientToken, async (req, res) => {
  ...
  if (state.db) {
    try {
      ...
      await state.withTimeout(updateDoc(doc(state.db, "submissions", subId), { status: "approved" }));
    } catch (e) {}
  }

  // Update card deck status
  ...
  if (state.db) {
    try {
      ...
      if (userDoc.exists()) {
        ...
        await state.withTimeout(updateDoc(userRef, { cv: currentPoints + points, points: currentPoints + points }));
      } else {
        await state.withTimeout(setDoc(userRef, { uid: sub.userId, name: sub.username, email: sub.userEmail, role: "Penonton Teater", cv: points, points }));
      }
    } catch (e) {}
  }

  // Trigger Discord DM or roles
...
app.post('/api/submissions/reject', requireClientToken, async (req, res) => {
  ...
  if (state.db) {
    try {
      ...
      await state.withTimeout(deleteDoc(doc(state.db, "submissions", subId)));
    } catch (e) {}
  }

  // Update card deck status to Denied
  if (sub.userId && sub.questId) {
    const decks = db.loadLocalDecks();
    if (decks[sub.userId]) {
      decks[sub.userId].statuses = decks[sub.userId].statuses || {};
...
app.post('/api/submissions/submit', requireClientToken, async (req, res) => {
  ...
  if (state.db) {
    try {
      ...
      await state.withTimeout(setDoc(doc(state.db, "submissions", newSub.id), newSub));
    } catch (e) {}
  }

  // Update card deck status to Review
  if (userId && questId) {
    const decks = db.loadLocalDecks();
    if (decks[userId]) {
  ...
  // Send review Embed to Discord admin channel
  if (state.client && state.isDiscordReady) {
    try {
      ...
      if (reviewChan && reviewChan.isTextBased()) {
        ...
        if (state.db) {
          try {
            const { doc, updateDoc } = require('firebase/firestore');
            await state.withTimeout(updateDoc(doc(state.db, "submissions", newSub.id), { discordMessageId: sentMessage.id }));
          } catch (e) {}
        }
    ...
    try {
      ...
      if (deckDoc.exists()) {
        ...
        if (deckData.statuses) {
          ...
          await state.withTimeout(updateDoc(deckRef, { statuses: deckData.statuses }));
        }
      }
    } catch (e) {}
  }
...
app.post('/api/submissions/reset-all', requireClientToken, async (req, res) => {
  ...
  if (state.db) {
    try {
      const { collection, getDocs, deleteDoc, doc, updateDoc } = require('firebase/firestore');
      const subsSnap = await state.withTimeout(getDocs(collection(state.db, "submissions")));
      for (const d of subsSnap.docs) {
        await state.withTimeout(deleteDoc(doc(state.db, "submissions", d.id)));
      }
      const decksSnap = await state.withTimeout(getDocs(collection(state.db, "user_decks")));
      for (const d of decksSnap.docs) {
        await state.withTimeout(updateDoc(doc(state.db, "user_decks", d.id), { dealt: false, cards: [], statuses: {} }));
      }
    } catch (e) {}
  }
...
app.post('/api/decks/deal', requireClientToken, async (req, res) => {
  ...
  if (state.db) {
    try {
      const { doc, setDoc } = require('firebase/firestore');
      await state.withTimeout(setDoc(doc(state.db, "user_decks", uid), deckData));
    } catch (e) {}
  }
  ...
  if (state.db) {
    try {
      ...
      if (deckDoc.exists()) {
        ...
        await state.withTimeout(updateDoc(deckRef, { statuses }));
      }
    } catch (e) {}
  }
...
// QUESTS CONFIGURATION ENDPOINTS
...
app.post('/api/quests', requireClientToken, async (req, res) => {
  ...
  if (state.db) {
    try {
      const { doc, setDoc } = require('firebase/firestore');
      await state.withTimeout(setDoc(doc(state.db, "quests", quest.id), quest));
    } catch (e) {}
  }
...
app.delete('/api/quests/:id', requireClientToken, async (req, res) => {
  ...
  if (state.db) {
    try {
      const { doc, deleteDoc } = require('firebase/firestore');
      await state.withTimeout(deleteDoc(doc(state.db, "quests", id)));
    } catch (e) {}
  }
  ...
  if (state.db) {
    try {
      ...
      for (const q of defaults) {
        await state.withTimeout(setDoc(doc(state.db, "quests", q.id), q));
      }
    } catch (e) {}
  }
...
app.post('/api/quests/delete-all', requireClientToken, async (req, res) => {
  ...
  if (state.db) {
    try {
      ...
      const snap = await state.withTimeout(getDocs(collection(state.db, "quests")));
      for (const d of snap.docs) {
        await state.withTimeout(deleteDoc(doc(state.db, "quests", d.id)));
      }
    } catch (e) {}
  }
...
// SYNC WIDGET METADATA
app.post('/api/oauth/update-stats/:id', async (req, res) => {
  const { id } = req.params;
  const accounts = db.loadLinkedAccounts();
  const acc = accounts[id];
  if (!acc) return res.status(404).json({ error: "User belum terhubung dengan OAuth2." });
...
// Start metadata background sync job (every 15 minutes)
setInterval(runMetadataSyncCycle, 900000);
setTimeout(runMetadataSyncCycle, 15000);

// Setup TikTok check watcher cron (every 5 minutes)
